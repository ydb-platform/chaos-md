...
`complete` с `command_failed`. Для blade `-C` считает `Status: Destroyed`
чистым состоянием. При отсутствии UID команда сначала проверяет SSH и сообщает,
что состояние экспериментов без локальных UID не подтверждено. Недоступный SSH,
живой эксперимент (`Success` и т.п.) или недоступный `status` завершают проверку
ошибкой. `-D` идемпотентен:
уже `Destroyed` снимает хвост UID и не считается leftover. Человекочитаемые
детали остаются в stderr. Строки JSON сохраняют UTF-8, обратные слеши и
управляющие символы.

Промежуточные командные кадры описывают только подтверждённо завершённые этапы.
Они используют `command_succeeded` и `exitCode=null`. Ошибка этапа появляется
только в итоговом кадре `complete` с ненулевым `exitCode`. Если компенсация или
снятие завершились успешно после другой ошибки, команда дополнительно выдаёт
успешный промежуточный кадр `teardown`.
...
Сигналы `INT` и `TERM` запускают тот же адресный teardown до завершения процесса.
Успешное снятие подтверждается одним кадром `teardown`, а терминальный кадр
возвращает ошибку с кодом 130 или 143. Если teardown завершается ошибкой,
удаленный таймер остается последней автоматической защитой.
...
блокировки и каждый вызов `tc`. Срок операции сохраняется один раз и не продлевается
повторным запуском. Подготовка, изменение и проверка qdisc используют общий бюджет
25 секунд и не начинают следующую команду после истечения срока. Команды netem
передаются через один пакетный файл на интерфейс. Перед применением скрипт принимает
только пустой, `mq` или `noqueue` root qdisc. Другой root qdisc требует отдельного
адаптера восстановления и блокирует операцию.

Таймер сохраняет PID, идентичность процесса, состояние и время последнего перехода
в каталоге операции. Наблюдение `active` сообщает `recovery_armed=true` только пока
работает тот же процесс таймера. После срока таймер получает блокировку отдельными
попытками. Каждая попытка снятия ограничена 25 секундами. Неудачная попытка сохраняет
`cleanup_failed`, освобождает блокировку и повторяется, пока каталог операции существует.
