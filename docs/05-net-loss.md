...
состояние через `tc qdisc show`. Срок операции не продлевается повторным запуском.
Проверка `active` подтверждает защиту только для живого процесса исходного таймера.
Подробный жизненный цикл и состояния таймера описаны в [тесте 04](04-net-delay.md).
