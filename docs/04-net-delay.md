...
ноды. Исходный срок записывается один раз и не продлевается повторным запуском.
Скрипт проверяет срок перед каждой следующей операцией `tc`. Таймер публикует свою
идентичность и состояние. Проверка активной операции не подтверждает
`recovery_armed`, если процесс таймера умер или сменился. Неудачное автоматическое
снятие остается в состоянии `cleanup_failed` и повторяется с отдельным ограниченным
бюджетом.
